...
//			result = HB_lookup(hmap, package);
		...
		int sentSize = generalSendBack(client_sock, sAllInOne.c_str(),
				sAllInOne.size(), fromAddr, 0, TCP);
	...
	case 3: {
		...
		if (package.virtualpath().empty()) {
			...
			if (operation_status != 0) {
				cout << "server: zhtinsert error: key = "
						<< package.virtualpath() << " ret = "
						<< operation_status << endl;
			}
...
//Workstealing functions
	...
	case 20: {
		...
		if (r <= 0) {
			cout
					<< "Insert_waitq: Server could not send acknowledgement to client: sendto r = "
					<< r << endl;
		}
	...
	case 22: {
		...
		if (r <= 0) {
			cout
					<< "Insert_readyq: Server could not send acknowledgement to client: sendto r = "
					<< r << endl;
		}
	...
	case 23: {
		...
		if (r <= 0) {
			cout
					<< "check_if_task_is_ready: Server could not send acknowledgement to client: sendto r = "
					<< r << endl;
		}
	...
	case 25: {
		...
		if (r <= 0) {
			cout
					<< "update ZHT: Server could not send acknowledgement to client: sendto r = "
					<< r << endl;
		}
	...
	case 14: {
		...
		if (num_task > 0) {
			...
			if (LOGGING) {
				log_fp << "Sending " << num_task << " tasks... to index "
						<< index << "\n";
			}
...
//					char buf[Env::MAX_MSG_SIZE];
						...
						if (count > 0) {
							...
							int pos = 0;
							int bytes_recd = count;
							memcpy(&buf[pos], tempbuf, count);
							pos = pos + count;
							if (bytes_recd != string_size) {
								while (bytes_recd < string_size) {
									memset(tempbuf, '\0', Env::MAX_MSG_SIZE);
									count = generalReveiveTCP(events[i].data.fd,
											tempbuf, sizeof tempbuf, 0); //cout << "Received size = " << count << endl;
