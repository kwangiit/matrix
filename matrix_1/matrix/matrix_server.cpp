...
start_thresh
= 1000000;
long end_poll = 100000;
long end_thresh = 10000000;
int diff_thresh = 500;
...
vector<vector<string> > tokenize(string input, char delim1, char delim2,
 ...
 */
int32_t Worker::get_max_load() {	//cout << " in max load " << endl;
//max_load_struct* new_max_load;
	...
	for (i = 0; i < num_neigh; i++) {//cout << " max load i = " << i << " index = " << neigh_index[i] << endl;
...
void Worker::choose_neigh() {
	...
	for (i = 0; i < num_neigh; i++) {
		//cout << " i = " << i << endl;
		ran = rand() % num_nodes;
		x = hostComp(svrclient.memberList.at(ran),
				svrclient.memberList.at(selfIndex));
...
int Worker::notify(ComPair &compair) {
	...
	nots += compair.second.size();
	log_fp << "nots = " << nots << endl;
	return update_ret;
