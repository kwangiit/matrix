...
int main(int argc, char* argv[]) {
	...
	{
		string str = string();
		mc.init(num_tasks, sleep_time, testClient, logging, index, atoi(argv[13]), atoi(argv[14]), str);
	}
	else
	{
		mc.init(num_tasks, sleep_time, testClient, logging, index, atoi(argv[13]), atoi(argv[14]), argv[15]);
	}
...
//cout << " 1 ";
	...
	/* wait until all the servers are up!*/
...
//cout << cmd << endl;
	...
	mc.initializeTasks(per_client_task, sleep_time, mode, max_tasks_per_package, testClient, DAG_choice, argc);
